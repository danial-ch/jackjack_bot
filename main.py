...
hellos = ["hi", "سلام", "hey", "wassap", "علیکم السلام", "درود", "چه سلامی چه علیکی", "باز تویی که"]
laugh_responses = ["می خندی؟", "کیر خر", "زهر مار", " کوفت", "مرض", "کیر"]
name_responses = ["جان", "بله", "بفرمایید", "جونم", "چیه", "چی میگی", "بگو", "زرتو بزن", "بنال", "در خدمتم", "بفرما"]
adjectives = ["قربان", "سکسی", "عزیزم", "دیوث", "کسکض", "اسکل خان", "الاغ", "لاش گوشت", "نفهم", "کله کیری", "سسکی",
              "کون گشاد", "باسنی", "اوبی", "چاقال"]
name_no_answer_responses = ["حال ندارم", "ولم کن", "چقد صدا میزنی منو", "بکشین بیرون از من", "گاییدین",
                            "بسه دیگه لاشیا", "عنشو در نیارین دیگه"]
passive_sentences = ["چفد گوه میخورین", "یه صلوات بفرستین", "این بالایی کونیه", "پاشین برین خونه هاتون",
                     "این پایینی کونیه"]
...
def empty_message(update, context):
    ...
    # if message.startwith("hi ") or message.startwith("hello ") or message.startwith("yo yo ") or \
    #         message.startwith("سلام ") or message.find("hello ") != -1:

    if check_with_list(message, ["hello", "hi", "salam", "slm", "yo yo ", "hey", "سلام"]):
        num = random.randint(0, len(hellos))
        send_to_async(context, update, hellos[num])

    if check_with_list(message, ["jack", "jack jack", "jacki", "جک", "جک جک", "جکی"]):
        if random.randint(0, 2) == 0:
            num = random.randint(0, len(name_no_answer_responses))
            send_to_async(context, update, name_no_answer_responses[num])
        else:
            num = random.randint(0, len(name_responses))
            if random.randint(0, 1) == 0:
                num2 = random.randint(0, len(adjectives))
                send_to_async(context, update, name_responses[num] + " " + adjectives[num2])
            else:
                send_to_async(context, update, name_responses[num])

    if message.find("😂😂😂😂😂") != -1 or message.find("😐😐😐😐😐") != -1:
        send_to_async(context, update, "کیییییییییر")

    if random.randint(0, 10) == 0:
        num = random.randint(0, len(passive_sentences))
        send_to_async(context, update, passive_sentences[num])

    if random.randint(0, 30) == 0:
        send_to_async(context, update, "گمشین دیگه کسکشششششا")

    if message.find("😂😂😂") != -1 or message.find("😐😐😐") != -1:
        if random.randint(0, 2) == 0:
            num = random.randint(0, len(laugh_responses))
            send_to_async(context, update, laugh_responses[num])

    if (message.startswith("ایا") or message.startswith("آیا")) and message.endswith("؟"):
        num = random.randint(0, 3)
        if num == 0:
            send_to_async(context, update, "بله")
        elif num == 1:
            send_to_async(context, update, "اره")
        elif num == 2:
            send_to_async(context, update, "نه")
        else:
            send_to_async(context, update, "خیر")
...
def send_to_async(context, update, message):
    chat_id = update.message.chat.id
    send_async(context,
               chat_id=chat_id,
               text=message,
...
def check_with_list(entry, check_text):
    for x in check_text:
        if entry.startswith(x) and entry.endswith(x):
            return True
...
def main():
    ...
    dp.add_handler(MessageHandler(Filters.update, empty_message))
